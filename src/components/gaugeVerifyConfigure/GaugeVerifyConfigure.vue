<template>
  <el-container>
    ...
    <el-main>
      <el-dialog title="编辑用能单位计量器具检定/校准记录" :visible.sync="dialogFormVisibleEdit" >
        <el-form ref="dialogFormEdit" :model="dialogFormEdit" :rules="rules" label-width="200px">
          ...
          <el-form-item prop="serno" label="计量器出产序号">
              <el-input v-model="dialogFormEdit.serno"></el-input>
          </el-form-item>
          <el-form-item prop="manageno" label="计量器管理编号">
              <el-input v-model="dialogFormEdit.manageno"></el-input>
          </el-form-item>
          <el-form-item prop="barcode" label="计量器二维码">
              <el-input v-model="dialogFormEdit.barcode"></el-input>
          </el-form-item>
          <el-form-item prop="verifyorg" label="校验机构">
              <el-input v-model="dialogFormEdit.verifyorg"></el-input>
          </el-form-item>
          <el-form-item label="校验时间" prop="verifytime">
            <el-col :span="8">
              <el-date-picker
                v-model="dialogFormEdit.verifytime"
                type="date"
                placeholder="选择日期"
      ...
      <el-dialog title="新增用能单位计量器具检定/校准记录" :visible.sync="dialogFormVisible" >
        <el-form ref="dialogForm" :model="dialogForm" label-width="200px" :rules="rules">
          ...
          <el-form-item prop="verifyorg" label="校验机构">
              <el-input v-model="dialogForm.verifyorg"></el-input>
          </el-form-item>
          <el-form-item label="校验时间" prop="verifytime">
            <el-col :span="8">
              <el-date-picker
                v-model="dialogForm.verifytime"
                type="date"
                placeholder="选择日期"
                :picker-options="pickerOptions"
                format="yyyy 年 MM 月 dd 日"
      ...
      <el-table :data="tableData.slice((page.currentPage-1)*page.pageSize,page.currentPage*page.pageSize)"  :row-class-name="tableRowClassName">
        ...
        <el-table-column prop="serno" label="计量器出产序号" width="200">
        </el-table-column>
        <el-table-column prop="manageno" label="计量器管理编号" width="200">
        </el-table-column>
        <el-table-column prop="barcode" label="计量器二维码" width="200">
        </el-table-column>
        <el-table-column prop="verifyorg" label="校验机构" width="200">
        </el-table-column>
        <el-table-column prop="verifytime" label="校验时间" width="200">
        </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules:{
          ...
          serno:[
            { required: true, message: '计量器出产序号', trigger: 'blur' },
            { min: 1, max: 30, message: '长度在 1 到 30 个字符', trigger: 'blur' }
          ],
          manageno:[
            { required: true, message: '计量器管理编号', trigger: 'blur' },
            { min: 12, max: 12, message: '长度在 12 个字符', trigger: 'blur' }
          ],
          verifyorg:[
            { required: true, message: '校验机构', trigger: 'blur' },
            { min: 1, max: 30, message: '长度在 1 到 30 个字符', trigger: 'blur' }
          ],
          verifytime:[
            { required: true, message: '校验时间', trigger: 'blur' },
            { min: 1, max: 30, message: '长度在 1 到 30 个字符', trigger: 'blur' }
          ]
        },
        tableData: [],
    ...
    methods: {
      saveAdd(formName){
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            self.$http.post(self.$baseUrl+"/gaugeVerifyConfigure/add",data,self.$config).then(
              res=>{
                console.log(res);
                if (res.data.code === 0){
                  self.$message({
                    message: '新增用能单位计量器具检定/校准记录成功',
                    type: 'success'
                  });
                }else {
                  self.$message({
      ...
      saveEdit(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            self.$http.post(self.$baseUrl+"/gaugeVerifyConfigure/update",data,self.$config).then(
              res=>{
                console.log(res);
                if (res.data.code === 0){
                  self.$message({
                    message: '更新用能单位计量器具检定/校准记录成功',
                    type: 'success'
                  });
                }else {
                  self.$message({
      ...
      getList(eCode){
        ...
        //获取数据
        self.$http.post(self.$baseUrl+"/gaugeVerifyConfigure/list",data).then(res=>{
          console.log(res);
          if (res.data.code === 0){
      ...
      handleClick(row) {
        ...
        self.dialogFormEdit.verifyorg = row.verifyorg;
        self.dialogFormEdit.verifytime = row.verifytime;
      ...
      deleteItem(row) {
        ...
        self.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          self.$http.post(self.$baseUrl+"/gaugeVerifyConfigure/delete",data,self.$config).then(
            res =>{
              console.log(res);
              if (res.data.code === 0){
                self.$message({
    ...
    created() {
      ...
      self.$http.post(self.$baseUrl+"/gaugeVerifyConfigure/getVersion",data,self.$config).then(res=>{
        console.log(res);
        if (res.data.code === 0) {
