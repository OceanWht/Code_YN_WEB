<template>
  <el-container style="margin-top: 5%">
    ...
    <el-main>
      <el-dialog title="编辑能源项目负责人信息" :visible.sync="dialogFormVisibleEdit">
        <el-form ref="dialogForm" :model="dialogForm" :rules="rules" label-width="200px">
          ...
          <el-form-item prop="monitor" label="姓名">
      ...
      <el-dialog title="新增能源项目负责人信息" :visible.sync="dialogFormVisible">
        <el-form ref="dialogForm" :model="dialogForm" :rules="rules" label-width="200px">
          <el-form-item prop="regversion" label="平台服务版本">
            <el-input v-model="dialogForm.regversion" :disabled="true"></el-input>
          </el-form-item>
      ...
      <el-table :data="tableData.slice((page.currentPage-1)*page.pageSize,page.currentPage*page.pageSize)"
        ...
        <el-table-column prop="monitor" label="姓名" width="200">
        </el-table-column>
    ...
    methods: {
      ...
      saveUpdate(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            self.$http.post(self.$baseUrl + "/CompanyEnergy/energyMonitorUpdate", data, self.$config).then(
              res => {
                console.log(res);
                if (res.data.code === 0) {
                  self.$message({
      ...
      saveAdd(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            self.$http.post(self.$baseUrl + "/CompanyEnergy/energyMonitorAdd", data, self.$config).then(
              res => {
                console.log(res);
                if (res.data.code === 0) {
                  self.$message({
      ...
      deleteItem(row) {
        ...
        self.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          self.$http.post(self.$baseUrl + "/CompanyEnergy/energyMonitorDelete", data, self.$config).then(
            res => {
              console.log(res);
              if (res.data.code === 0) {
                self.$message({
      ...
      handleClick(row) {
        ...
        self.dialogForm.monitor = row.monitor;
        self.dialogForm.dept = row.dept;
        self.dialogForm.phone = row.phone;
        self.dialogForm.job = row.job;
      ...
      getList(code) {
        ...
        //获取数据
        self.$http.post(self.$baseUrl + "/CompanyEnergy/energyMonitorList", data, self.$config).then(res => {
          console.log(res);
          if (res.data.code === 0) {
            if (res.data.result) {
