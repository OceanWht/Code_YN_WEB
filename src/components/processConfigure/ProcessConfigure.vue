<template>
  <el-container style="margin-top: 5%">
    ...
    <el-main>
      <el-dialog title="编辑生产工序信息" :visible.sync="dialogFormVisibleEdit">
        <el-form ref="dialogFormEdit" :model="dialogFormEdit" :rules="rules" label-width="200px">
          ...
          <el-form-item prop="processCode" label="生产工序编码">
            <el-input v-model="dialogFormEdit.processCode"></el-input>
          </el-form-item>
          <el-form-item prop="processName" label="生产工序名称">
            <el-input v-model="dialogFormEdit.processName"></el-input>
          </el-form-item>
      ...
      <el-dialog title="新增生产工序信息" :visible.sync="dialogFormVisible">
        <el-form ref="dialogForm" :model="dialogForm" :rules="rules" label-width="200px">
          ...
          <el-form-item prop="processName" label="生产工序名称">
            <el-input v-model="dialogForm.processName"></el-input>
          </el-form-item>
          <el-form-item prop="remark" label="备注">
      ...
      <el-table :data="tableData.slice((page.currentPage-1)*page.pageSize,page.currentPage*page.pageSize)"
        ...
        <el-table-column prop="processcode" label="生产工序编码" width="200">
        </el-table-column>
        <el-table-column prop="processname" label="生产工序名称" width="200">
        </el-table-column>
    ...
    export default {
      ...
      data() {
        return {
          ...
          rules: {
            processCode: [
              {required: true, message: '请输入生产工序编码', trigger: 'blur'},
              {min: 2, max: 11, message: '长度在 1 到 11 个字符', trigger: 'blur'}
            ],
            processName: [
              {required: true, message: '请输入生产工序名称', trigger: 'blur'},
      ...
      methods: {
        ...
        deleteItem(row) {
          ...
          self.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
            ...
            self.$http.post(self.$baseUrl + "/processConfigure/delete", data, self.$config).then(
              res => {
                console.log(res);
                if (res.data.code === 0) {
          ...
          self.$refs[formName].validate((valid) => {
            if (valid) {
              let data = {
                ...
                'processName': self.dialogFormEdit.processName,
                'remark': self.dialogFormEdit.remark
              }
              self.$http.post(self.$baseUrl + "/processConfigure/update", data, self.$config).then(
                res => {
                  console.log(res);
                  if (res.data.code === 0) {
                    if (res.data.result.responseCode === 200) {
          ...
          self.$refs[formName].validate((valid) => {
            if (valid) {
              let data = {
                ...
                'processName': self.dialogForm.processName,
                'remark': self.dialogForm.remark
              }
              self.$http.post(self.$baseUrl + "/processConfigure/add", data, self.$config).then(
                res => {
                  console.log(res);
                  if (res.data.code === 0) {
                    if (res.data.result.responseCode === 200){
        ...
        handleClick(row) {
          ...
          self.dialogFormEdit.processName = row.processname;
        ...
        getList(code){
          ...
          //获取数据
          self.$http.post(self.$baseUrl + "/processConfigure/list", data, self.$config).then(res => {
            console.log(res);
            if (res.data.code === 0) {
              if (res.data.result) {
